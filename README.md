...
2. Either use the hotkey `Ctrl/cmd + shift + =` (this can be changed via Hotkeys settings, change it to `Ctrl + shift + +` on Windows), or press `Ctrl/cmd + p` to open the settings dialog and search for `Wikilinks to MDLinks: Toggle selected wikilink to markdown link and vice versa`. Just typing in a bit of the command, e.g. "toggle" would likely allow this command to show up via the suggestions the search dialog provides.
